...
const item: Variants = {
  ...
  show: {
    ...
    transition: {
      duration: 0.6,
      ease: [0.25, 0.1, 0.25, 1], // ✅ FIX TS (pas de string)
    },
  },
};
  ...
  return (
    <section className="relative overflow-hidden py-32 bg-white">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6">
        ...
        >
          <motion.p variants={item} className="mb-4 text-xs font-semibold uppercase tracking-[0.35em] text-black/40">
            Studio
          </motion.p>

          <motion.h2 variants={item} className="text-5xl md:text-6xl font-black tracking-tight text-black">
            Design, technologie & performance.
          </motion.h2>

          <motion.p variants={item} className="mt-6 text-lg text-black/60">
        ...
        <div className="grid gap-6 md:grid-cols-3 auto-rows-[260px]">
          ...
          {/* CARD 1 */}
          <motion.div variants={item} whileHover={{ y: -10 }} className="group relative overflow-hidden rounded-[40px] border border-black/10 bg-white/70 p-8 backdrop-blur-2xl transition hover:shadow-2xl">
            ...
            <p className="mt-4 text-black/60">Interfaces modernes centrées utilisateur et conversion.</p>
          </motion.div>
          ...
          <motion.div variants={item} whileHover={{ y: -10 }} className="group relative overflow-hidden rounded-[40px] border border-black/10 bg-white/70 p-8 backdrop-blur-2xl transition hover:shadow-2xl">
            ...
            <p className="mt-4 text-black/60">Optimisation poussée pour vitesse et SEO.</p>
          </motion.div>

          {/* LARGE CARD */}
          <motion.div variants={item} whileHover={{ y: -10 }} className="group relative col-span-2 overflow-hidden rounded-[40px] border border-black/10 bg-white/70 p-10 backdrop-blur-2xl transition hover:shadow-2xl">
            ...
            <button className="mt-10 flex items-center gap-3 rounded-full border border-black/10 px-7 py-4 text-sm font-semibold transition hover:bg-black hover:text-white">
